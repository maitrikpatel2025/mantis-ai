...
import type { RegistrySkill, RegistryData } from './registry.js';
...
interface SkillManifest {
  registry: string;
  installed: Record<string, SkillManifestEntry>;
}
...
export function loadManifest(): SkillManifest {
  try {
    if (fs.existsSync(skillsFile)) {
      return JSON.parse(fs.readFileSync(skillsFile, 'utf8')) as SkillManifest;
    }
  } catch {}
 ...
 */
export async function installSkill(name: string): Promise<SkillOperationResult> {
  const manifest = loadManifest();
  const registryUrl = manifest.registry || DEFAULT_REGISTRY;

  // Fetch registry and find skill
  const registry = await fetchRegistry(registryUrl);
  const skill = registry.skills?.find((s: { name: string }) => s.name === name);
  if (!skill) {
 ...
 */
export function removeSkill(name: string): SkillOperationResult {
  const manifest = loadManifest();
 ...
 */
export function toggleSkill(name: string, enabled: boolean): SkillOperationResult {
  const manifest = loadManifest();
  const symlinkPath = path.join(piSkillsDir, name);
  const sourcePath = path.join(piSkillsSourceDir, name);
 ...
 */
export async function checkUpdates(): Promise<SkillUpdateInfo[]> {
  const manifest = loadManifest();
  const registryUrl = manifest.registry || DEFAULT_REGISTRY;
  ...
  for (const [name, info] of Object.entries(manifest.installed || {})) {
    ...
    const remote = registry.skills?.find((s: RegistrySkill) => s.name === name);
