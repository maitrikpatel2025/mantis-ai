...
function createWorkspaceTools() {
  ...
  const runCommandTool = tool(
    ...
    {
      name: "run_command",
      description: "Execute a shell command instantly in your persistent workspace (a Docker container with Node.js, git, Chrome, and common tools). Use for quick tasks: downloading files, running scripts, data processing, API calls, git operations, web scraping. The workspace persists between calls \u2014 installed packages, created files, and environment state all remain. For heavy autonomous coding tasks that should produce a PR, use create_job instead.",
      schema: z.object({
  ...
  const readFileTool = tool(
    ...
    },
    {
      ...
      description: "Read a file from the workspace container. Path is relative to /workspace. Use to inspect command outputs, check configs, or read downloaded files.",
      schema: z.object({
  ...
  const writeFileTool = tool(
    ...
    },
    {
      ...
      description: "Write a file to the workspace container. Path is relative to /workspace. Directories are created automatically. Use to create scripts, configs, or data files.",
      schema: z.object({
        path: z.string().describe("File path relative to /workspace"),
  ...
  const installPackageTool = tool(
    ...
    },
    {
      ...
      description: "Install packages in the workspace container. Supports npm packages (default) and apt system packages. Installed packages persist for the lifetime of the workspace. Use when a command needs a tool that is not pre-installed.",
      schema: z.object({
