...
class DiscordAdapter extends ChannelAdapter {
   ...
   */
  constructor(botToken, applicationId, publicKey, channelConfig) {
    super(channelConfig);
  ...
  async receive(request) {
    ...
    if (body.type === 2) {
      ...
      const senderId = body.member?.user?.id || body.user?.id || "";
      const isGroup = !!body.guild_id;
...
${transcription}` : transcription;
  ...
  startProcessingIndicator(metadata) {
    ...
    let timeout;
    let stopped = false;
    const schedule = (botToken, channelId) => {
      if (stopped) return;
      timeout = setTimeout(() => {
        if (stopped) return;
