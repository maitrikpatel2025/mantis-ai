...
async function runJobLocally(jobId, branch) {
...
function cancelLocalJob(jobId) {
  const entry = activeJobs.get(jobId);
  if (entry) {
    try {
      execSync(`docker stop ${entry.containerName}`, { stdio: "ignore", timeout: 15e3 });
    } catch {
      try {
...
async function cleanupOrphanedContainers() {
  try {
    ...
    for (const line of lines) {
      ...
      try {
        execSync(`docker stop ${containerName}`, { stdio: "ignore", timeout: 15e3 });
