...
class WorkspaceManager {
  ...
  async _start() {
    ...
    const args = [
      ...
      CONTAINER_NAME,
      "--label",
      "mantis-workspace=true",
      "-p",
      `${port}:8080`,
      "-v",
      `${workerScript}:/workspace-worker.js:ro`,
      "-e",
  ...
  async _cleanupOrphan() {
    try {
      execSync(`docker rm -f ${CONTAINER_NAME}`, { stdio: "ignore", timeout: 15e3 });
    } catch {
    }
  }
  /**
  ...
  getStatus() {
    return {
      ...
      idleSeconds: this.lastActivityAt ? Math.floor((Date.now() - this.lastActivityAt) / 1e3) : 0
    };
  }
  /**
  ...
  async shutdown() {
    ...
    try {
      execSync(`docker stop ${CONTAINER_NAME}`, { stdio: "ignore", timeout: 15e3 });
    } catch {
    }
    try {
      execSync(`docker rm -f ${CONTAINER_NAME}`, { stdio: "ignore", timeout: 5e3 });
    } catch {
    }
    this.status = "stopped";
    this.containerId = null;
  ...
  _fetchHealth(host, port) {
    return new Promise((resolve2, reject) => {
      const req = http.get(`http://${host}:${port}/health`, { timeout: 5e3 }, (res) => {
        let data = "";
        res.on("data", (c) => {
          data += c;
        });
...
async function shutdownWorkspace() {
  const ws = globalThis.__mantis_workspace;
  if (ws) {
    await ws.shutdown();
