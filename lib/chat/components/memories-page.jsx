...
function MemoryCard({ memory, onUpdate, onDelete }) {
  ...
  const handleSave = async () => {
    ...
    try {
      await onUpdate(memory.id, {
        ...
        relevance: parseInt(editRelevance, 10),
      });
      setEditing(false);
    } catch {} finally {
      setSaving(false);
...
export function MemoriesPage({ session }) {
  ...
  const fetchMemories = useCallback(async () => {
    try {
      ...
      let result;
      if (search.trim()) {
        result = await searchMemoriesAction(search.trim(), cat);
      } else {
        result = await getMemoriesAction(cat);
      }
      setMemories(result || []);
    } catch {
      setMemories([]);
  ...
  const handleCreate = async (content, cat) => {
    const result = await createMemoryAction(content, cat);
    if (!result.error) {
      await fetchMemories();
    }
  };
