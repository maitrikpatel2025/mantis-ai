...
function groupChatsByDate(chats: ChatData[]): Record<string, ChatData[]> {
  const now = new Date();
...
function ChatItem({ chat, isActive, onSelect, onDelete, onStar, onRename }: ChatItemProps) {
...
export function ConversationsPanel({ open, onClose }: ConversationsPanelProps) {
  const [chats, setChats] = useState<ChatData[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [search, setSearch] = useState<string>('');
  const chatNav = useChatNav();
  const activeChatId = chatNav?.activeChatId ?? null;
