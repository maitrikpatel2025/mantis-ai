...
interface SkillCardProps {
  ...
  onRemove: ((name: string) => Promise<void>) | null;
}
...
function RegistrySkillCard({ skill, installed, onInstall }: RegistrySkillCardProps) {
...
function UpdateCard({ update, onUpdate }: UpdateCardProps) {
...
interface SkillsPageProps {
  ...
  searchSkillsAction: (query: string) => Promise<RegistrySkill[]>;
  installSkillAction: (name: string) => Promise<ActionResult>;
  toggleSkillAction: (name: string, enabled: boolean) => Promise<ActionResult>;
  removeSkillAction?: (name: string) => Promise<ActionResult>;
  checkSkillUpdatesAction?: () => Promise<SkillUpdate[]>;
}
...
export function SkillsPage({
  ...
  const [searchResults, setSearchResults] = useState<RegistrySkill[] | null>(null);
  ...
  async function handleRemove(name: string) {
    ...
    if (result?.success) {
      showStatus(result.message || 'Removed');
  ...
  async function handleUpdate(name: string) {
    const result = await installSkillAction(name);
    if (result?.success) {
      showStatus(result.message || 'Updated');
