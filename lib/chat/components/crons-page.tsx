...
const typeOrder: Record<string, number> = { agent: 0, command: 1, webhook: 2 };
...
function sortByType(items: CronItem[]): CronItem[] {
...
const EMPTY_FORM: CronFormData = { name: '', schedule: '', type: 'agent', job: '', command: '', url: '', method: 'POST', enabled: true };

interface CronFormModalProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: CronFormData) => Promise<{ success?: boolean; message?: string }>;
  initial: CronItem | null;
  title?: string;
}

function CronFormModal({ open, onClose, onSubmit, initial, title }: CronFormModalProps) {
  ...
  useEffect(() => {
    if (open) {
      setForm(initial ? { ...EMPTY_FORM, ...initial } as CronFormData : EMPTY_FORM);
      setError('');
    }
  }, [open]);
  ...
  const set = (key: keyof CronFormData, value: string | boolean) => setForm((f) => ({ ...f, [key]: value }));
...
function CronRunHistory({ cronName }: CronRunHistoryProps) {
  ...
  // Live updates
  useEventStream('cron:run', useCallback((data: any) => {
...
interface CronCardProps {
  cron: CronItem;
  index: number;
  stats: Record<string, CronRunStats>;
  onEdit: (index: number) => void;
  onDelete: (index: number) => void;
  onToggle: (index: number) => void;
}

function CronCard({ cron, index, stats, onEdit, onDelete, onToggle }: CronCardProps) {
...
export function CronsPage() {
  ...
  const [stats, setStats] = useState<Record<string, CronRunStats>>({});
  ...
  const loadStats = () => {
    getCronRunStatsAction()
      .then((data: any[]) => {
        const map: Record<string, CronRunStats> = {};
  ...
  const handleCreate = async (data: CronFormData) => {
    const result = await createCron(data as any);
    if (result.success) reload();
    return result;
  };

  const handleEdit = async (data: CronFormData) => {
    const result = await updateCron(editIndex!, data as any);
    if (result.success) reload();
    return result;
  };
  ...
  const getOriginalIndex = (cron: CronItem): number => crons.indexOf(cron);
