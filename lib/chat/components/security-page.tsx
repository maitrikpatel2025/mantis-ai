...
interface SecurityPageProps {
  getSecurityPoliciesAction: () => Promise<Policy[]>;
  updateToolPolicyAction: (agent: string, tool: string, policy: string) => Promise<void>;
  getPendingApprovalsAction: () => Promise<Approval[]>;
  respondToApprovalAction: (id: string, approved: boolean) => Promise<void>;
  getToolNamesAction: () => Promise<string[]>;
}
...
export function SecurityPage({
  ...
  const handlePolicyChange = async (agent: string, tool: string, policy: string) => {
  ...
  const policyGroups: Record<string, Policy[]> = {};
      ...
      <div className="mb-6">
        ...
        <div className="flex items-end gap-2 p-4 rounded-xl border bg-card shadow-xs">
          ...
          <div>
            ...
            <select
              value={newTool}
              onChange={(e: ChangeEvent<HTMLSelectElement>) => setNewTool(e.target.value)}
          ...
          <div>
            ...
            <select
              value={newPolicy}
              onChange={(e: ChangeEvent<HTMLSelectElement>) => setNewPolicy(e.target.value)}
              className="mt-1 rounded-lg border border-input bg-transparent px-2 py-1.5 text-xs shadow-xs focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-ring/50 focus-visible:border-ring transition-[color,box-shadow]"
            >
                  ...
                  {agentPolicies.map((p, i) => (
                    <div key={i} className="flex items-center justify-between px-4 py-2.5 hover:bg-accent/30 transition-colors">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <select
                          value={p.policy}
                          onChange={(e: ChangeEvent<HTMLSelectElement>) => handlePolicyChange(p.agent, p.tool, e.target.value)}
