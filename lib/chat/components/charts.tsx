...
export function AreaChart({ data = [], height = 80, color = 'emerald', formatValue }: AreaChartProps) {
...
export function DonutChart({ data = [], size = 120, innerRadius = 0.6 }: DonutChartProps) {
  const [hoverIdx, setHoverIdx] = useState<number | null>(null);
  ...
  const arcs: DonutArc[] = data.map((d, i) => {
...
const BAR_COLORS: Record<string, BarColorSet> = {
...
export function BarChart({ data = [], height = 128, color = 'emerald', formatValue, label }: BarChartProps) {
  const [hoverIdx, setHoverIdx] = useState<number | null>(null);
...
interface TokenBreakdownBarsProps {
  data?: TokenBreakdownEntry[];
}

export function TokenBreakdownBars({ data = [] }: TokenBreakdownBarsProps) {
  if (!data.length) {
    return <p className="text-sm text-muted-foreground">No data</p>;
  }
...
interface SourceBreakdownBarsProps {
  data?: SourceBreakdownEntry[];
}

export function SourceBreakdownBars({ data = [] }: SourceBreakdownBarsProps) {
  if (!data.length) {
    return <p className="text-sm text-muted-foreground">No data</p>;
  }
