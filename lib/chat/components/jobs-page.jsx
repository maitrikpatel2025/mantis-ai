...
const FILTER_STATUSES = ['all', 'created', 'queued', 'completed', 'failed'];
...
export function JobsPage({ session }) {
  const [jobs, setJobs] = useState([]);
  const [counts, setCounts] = useState({ created: 0, queued: 0, completed: 0, failed: 0 });
  const [filter, setFilter] = useState('all');
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);

  const fetchData = useCallback(async (p, status) => {
    try {
      ...
      setJobs(jobList || []);
      setCounts(jobCounts || { created: 0, queued: 0, completed: 0, failed: 0 });
      setPage(p);
    } catch (err) {
