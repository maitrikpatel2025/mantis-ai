...
export async function register(): Promise<void> {
  ...
  try {
    ...
    const stored: string | null = getAvailableVersion();
  ...
  try {
    ...
    console.error('[mantis] Orphaned container cleanup failed:', (err as Error).message);
  }
  ...
  try {
    ...
    console.error('[mantis] Warm pool initialization failed:', (err as Error).message);
  }
  ...
  if (process.env.GATEWAY_ENABLED === 'true') {
    ...
    try {
      ...
      console.error('[mantis] Gateway startup failed:', (err as Error).message);
    }
  }
  ...
  try {
    ...
    console.error('[mantis] Route pre-compilation failed:', (err as Error).message);
  }
